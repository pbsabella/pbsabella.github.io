...
export const TokenTable = ({ tokens, showValue = false }: { tokens: TokenRow[]; showValue?: boolean }) => {
  ...
  const rows = tokens.map((token) => {
    const baseCells = [
      ...
      />,
      token.name,
      <code className="code" key={`${token.token}-token`}>{token.token}</code>,
    ];
    ...
    return [...baseCells, <code className="code" key={`${token.token}-value`}>{resolvedValue || '—'}</code>];
  });
  ...
  <div className={styles.tokenTable}>
    <Table
      ...
      rows={SPACING_SCALE.map((item) => [
        ...
        <code className="code" key={`${item.token}-token`}>{item.token}</code>,
        item.value,
      ])}
    />
  </div>
    ...
    <Table
      ...
      rows={TYPE_SCALE.map((type, idx) => {
        ...
        return [
          ...
          <code className="code" key={`${type.token}-token`}>{type.token}</code>,
          <code className="code" key={`${type.token}-leading`}>{type.leading}</code>,
        ];
      })}
    />
  </div>
  ...
  <div className={styles.tokenTable}>
    <Table
      ...
      rows={FONT_WEIGHTS.map((weight) => [
        ...
        <code className="code" key={`${weight.token}-token`}>{weight.token}</code>,
      ])}
    />
  </div>
);
