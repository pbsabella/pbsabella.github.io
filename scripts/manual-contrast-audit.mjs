 ...
 *
 * What it does:
 * - Scans common text elements on each configured route/theme/brand.
 ...
 *
 * Output:
 * - Per route/theme/brand: scanned count, fail count, unknown background count.
...
const BRANDS = ['portfolio', 'fintech', 'saas', 'editorial', 'neon'];
...
for (const brand of BRANDS) {
  for (const theme of THEMES) {
    for (const route of ROUTES) {
      ...
      await page.evaluate((brandValue) => {
      ...
      await page.waitForTimeout(350); // allow CSS transitions to settle after brand switch
      ...
      summary.push({ brand, theme, route, ...result, failCount: result.findings.length });
      await context.close();
    }
  }
}
...
console.log(`${color('Routes/Themes/Brands:', ANSI.dim)} ${ROUTES.length}/${THEMES.length}/${BRANDS.length}`);
console.log(color('==========================================', ANSI.dim));

for (const brand of BRANDS) {
  console.log(`\n${color(`Brand: ${brand.toUpperCase()}`, ANSI.cyan)}`);
  console.log(color('------------------------------------------', ANSI.dim));

  const brandRows = summary.filter((row) => row.brand === brand);
  for (const theme of THEMES) {
    console.log(`\n${color(`Theme: ${theme.toUpperCase()}`, ANSI.dim)}`);
    const themeRows = brandRows.filter((row) => row.theme === theme);
